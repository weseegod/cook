//! End-to-end tests for `install_internal` — the fork's release-feed
//! installer used when `installer = "internal"` is configured.
//!
...
#[tokio::test]
#[serial]
async fn install_internal_fails_when_managed_link_is_blocked() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
