...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`thanh setup`): explicit `managed_config_url`,
...
/// (`thanh workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `thanh workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// by `thanh setup`) sit under the env vars, requirements pins are applied on
