...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`cook setup`): explicit `managed_config_url`,
...
/// (`cook workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `cook workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// by `cook setup`) sit under the env vars, requirements pins are applied on
