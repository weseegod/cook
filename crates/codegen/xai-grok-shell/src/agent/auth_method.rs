...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `cook login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `cook login`, set XAI_API_KEY, or add api_key to ~/.cook/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `cook login` to authenticate.";
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.cook/auth.json".to_string())),
    )
}
...
mod tests {
    ...
    /// `~/.cook/config.toml` skeleton from the bug report, walks it through
    ...
    // -- cook login --legacy regression coverage ------------------------
    //
    // `cook login --legacy` produces a GrokAuth with `auth_mode: WebLogin`,
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        // Construct a legacy-style token exactly as `cook login --legacy`
