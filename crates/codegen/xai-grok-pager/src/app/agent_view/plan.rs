...
impl AgentView {
    ...
    pub(super) fn handle_plan_feedback_key(&mut self, key: &KeyEvent) -> InputOutcome {
        ...
        // A leading-slash Enter is left to `route_enter` below, which decides
        // between running the command (`/model grok-4.5` — the overlay stays
        // open so `a`/`g` still work) and sending the text as revision notes.
        let leading_slash = self.prompt.text().trim_start().starts_with('/');
        if !is_commenting && self.prompt.slash_open() && !self.prompt.file_search_visible() {
            match key.code {
                ...
                KeyCode::Enter if key.modifiers.is_empty() && !leading_slash => {
                    // Only mid-text `/` tokens reach here: accept the
                    // completion instead of submitting the raw text.
                    let chains = self
                        .prompt
                        .slash_snapshot()
                        .selection()
...
mod plan_approval_slash_tests {
    ...
    /// Bare `/` on the parked-plan preview starts a slash command on the
    /// prompt (`/model`) instead of opening in-plan search. Search stays on `f`.
    #[test]
    fn plan_preview_slash_starts_command_on_prompt() {
        let mut agent = agent_with_slash_prompt();
        {
            let pav = agent.plan_approval_view.as_mut().expect("approval open");
            pav.focus = PlanApprovalFocus::Preview;
        }
        agent.show_plan_preview();
        assert!(agent.line_viewer.is_some(), "preview must be open");
        let slash = KeyEvent::new(KeyCode::Char('/'), KeyModifiers::NONE);
        let outcome = agent.handle_line_viewer_key(&slash);
        assert!(
            matches!(outcome, InputOutcome::Changed),
            "bare / on the plan preview must be consumed"
        );
        assert_eq!(
            agent.plan_approval_view.as_ref().map(|p| p.focus),
            Some(PlanApprovalFocus::Prompt),
            "/ must move focus to the approval prompt"
        );
        assert_eq!(agent.prompt.text(), "/");
        assert!(
            agent.prompt.slash_open(),
            "/ on the plan preview must open the slash dropdown on the prompt"
        );
        assert!(
            agent.plan_approval_view.is_some(),
            "the plan overlay must stay open for a/g"
        );
    }
