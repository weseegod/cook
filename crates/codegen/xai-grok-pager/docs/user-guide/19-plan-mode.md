...
The plan is written to `plan.md` inside the session directory (`~/.thanh/sessions/<cwd>/<session-id>/plan.md`, where `<cwd>` is an encoded directory name, not the literal path).
...
- Subagent spawns are gated by the same read-only rule. During plan mode the task tool only accepts subagent types that resolve to a genuinely read-only definition (for example `explore`): spawning one of these — several in parallel, for codebase exploration and read-only measurements — is allowed, and the child stays limited by its own toolset. Spawning a write-capable subagent (`general-purpose`, `codex`, or any type whose toolset includes edit/bash tools) is rejected before it runs, with a short message naming the plan file as the only editable path and redirecting to read-only types. Unknown subagent types are also rejected (fail-closed). Write-capable subagent work is deferred until after exiting plan mode.
