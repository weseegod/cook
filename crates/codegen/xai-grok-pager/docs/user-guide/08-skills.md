...
Skills are reusable prompt packages that extend Cook with task-specific instructions. They let you capture a repeatable procedure once, instead of re-explaining it each session. Startup discovery skips project skills and commands in untrusted folders.

---

## What Are Skills?

A skill is a directory that contains a `SKILL.md` file. Its markdown body tells Cook how to handle a specific type of task: step-by-step instructions, conventions, and tool-usage patterns.

Use a skill for a repeatable procedure that's too specific for AGENTS.md but too long to retype. Cook activates a skill only when it applies to your current task.

---

## Skill Locations

Cook discovers skills from these directories, in priority order:
...
Cook deduplicates skills by name -- a higher-priority location overrides a lower one. Cook also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.grok/`) and walks every directory between your working directory and the repo root.
...
Cook scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.cook/config.toml`, or set the `GROK_CURSOR_SKILLS_ENABLED` or `GROK_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. Cook always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.
...
| Field | Description |
|-------|-------------|
| `name` | Skill identifier. Use lowercase letters, digits, and hyphens, up to 64 characters. Cook normalizes spaces and underscores to hyphens. If you omit `name`, Cook uses the skill's directory name. |
| `description` | What the skill does and when to use it. Cook reads this to decide whether to invoke the skill. If you omit it, Cook uses the first paragraph of the body. |

Write a specific `description`. It determines when Cook invokes the skill automatically. Name the trigger phrases and use cases.
...
| `metadata` | Arbitrary string key-value pairs. Cook promotes `metadata.author` and `metadata.short-description` for display. |

---

## Creating Skills with /create-skill

The `/create-skill` command walks you through building a new skill interactively. Cook asks what you want, drafts the files, and writes them to disk.

### How It Works

When you run `/create-skill`, Cook:

1. **Gathers requirements.** Cook asks for the skill name, the scope to save it under, and a description of the workflow you want to capture. Use a name with lowercase letters, digits, and hyphens (2–64 characters, starting and ending with a letter or digit).

2. **Drafts the description.** Cook writes a `description` that states what the skill does, the phrases that trigger it, and the slash command name. You approve or edit the draft before continuing.

3. **Creates the skill directory.** Cook creates the `<scope>/.grok/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.

4. **Writes SKILL.md.** Cook writes the frontmatter (`name` and `description`) and a markdown body of instructions, along with any supporting files.

5. **Verifies and confirms.** Cook reads the file back, confirms it wrote correctly, and tells you how to run the skill.

### Choosing a Scope

Cook asks where to save the skill:

- **Project** (`<repo_root>/.grok/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Cook recommends this scope inside a git repository.
...
The new skill appears in the slash menu within a few seconds, because Cook reloads skills when files change on disk.
...
To browse your skills, type `/` to open the slash-command menu. Cook lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `cook inspect` (see [Viewing Skill Details](#viewing-skill-details)).

### Qualified Names

When a skill's name collides with another skill or a built-in command, Cook keeps **both** invocable. The built-in keeps the bare name (`/login`, `/compact`, …). The skill is advertised under a qualified name prefixed by its scope -- `local:`, `repo:`, `user:`, or the plugin name:
...
Run `cook inspect` to see every skill Cook discovers, along with the rest of your configuration:
...
Cook distributes platform skills separately from your personal skills. Bundled skills are cached under `~/.cook/bundled/skills/`; Cook never writes them into `~/.cook/skills/`. A same-named local, repo, or user skill overrides the bundled copy. `cook inspect` labels each definition by its actual source. (A plugin skill of the same name does not override a native skill; it stays available under its qualified `plugin:name` form.)
...
2. **Include concrete steps.** Skills work best when they give Cook a clear, ordered procedure to follow.
...
7. **Keep the body under the file-read cap.** Cook inlines at most the first 25,000 tokens of a skill body (the same cap as `read_file`). Put long reference material in sibling files and tell Cook to read them with a line offset and limit.
