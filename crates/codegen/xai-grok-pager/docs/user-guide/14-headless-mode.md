...
cook -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
cook -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
cook -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
cook -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
cook -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
cook -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
cook -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
cook -p "Build the project" --allow "Bash"
```
...
Like `streaming-json`, this stream is read only. Tool approvals and other bidirectional flows use the ACP interface (`cook agent`).

---

## Session Management in Headless Mode

By default, each `cook -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
cook -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
cook -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
cook -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
cook -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
cook -p "What's the secret number?" --resume abc123
```
...
```bash
cook -p "Continue where we left off" -c
```
...
cook -p "Generate a README" > README.md

# Parse JSON output with jq
cook -p "List files" --output-format json | jq -r '.text'
```
...
cook -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
cook --prompt-file ./prompt.txt
```
...
```bash
cook -p "Review changes for bugs and security issues." \
...
```bash
cook -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
```bash
cook -p "Format all files" --always-approve
cook -p "Run the tests and fix any failures" --cwd ~/projects/my-app --always-approve
```
...
cook -p "Run the test suite" --yolo
```
...
- **`cook login --device-auth`** (or `--device-code`): no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`cook login`**: browser-based OAuth2 on machines with a GUI.
...
For containers or CI, mount `~/.cook` read-only:
...
- Resume: `cook -p "continue" --resume "<id>"` or `cook -p "continue" --continue`
