...
thanh -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
thanh -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
thanh -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
thanh -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
thanh -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
thanh -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
thanh -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
thanh -p "Build the project" --allow "Bash"
```
...
Like `streaming-json`, this stream is read only. Tool approvals and other bidirectional flows use the ACP interface (`thanh agent`).

---

## Session Management in Headless Mode

By default, each `thanh -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
thanh -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
thanh -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
thanh -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
thanh -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
thanh -p "What's the secret number?" --resume abc123
```
...
```bash
thanh -p "Continue where we left off" -c
```
...
thanh -p "Generate a README" > README.md

# Parse JSON output with jq
thanh -p "List files" --output-format json | jq -r '.text'
```
...
thanh -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
thanh --prompt-file ./prompt.txt
```
...
```bash
thanh -p "Review changes for bugs and security issues." \
...
```bash
thanh -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
```bash
thanh -p "Format all files" --always-approve
thanh -p "Run the tests and fix any failures" --cwd ~/projects/my-app --always-approve
```
...
thanh -p "Run the test suite" --yolo
```
...
- **`thanh login --device-auth`** (or `--device-code`): no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`thanh login`**: browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `thanh -p "..." --yolo --rules "Never delete files"`.
...
Grok stores data in `~/.thanh` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.thanh` read-only:
...
- Resume: `thanh -p "continue" --resume "<id>"` or `thanh -p "continue" --continue`
