...
/// `~/.cook` or `$GROK_HOME`, decided by the resolved home rather than by
...
/// User-facing path under [`grok_home()`], e.g. ``~/.cook/config.toml``.
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        ...
        assert_eq!(display_grok_home_prefix(), "~/.cook");
    }
    ...
    fn display_user_grok_path_joins_relative() {
        ...
        assert!(path.contains(".cook") || path.contains("$GROK_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        ...
        let full = format!("{home}/.cook/memory/MEMORY.md");
