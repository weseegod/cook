...
impl SamplingClient {
    ...
    async fn chat_completion_stream_inner(
        ...
        // Gateway side events (e.g. OpenCode Zen's post-`[DONE]` billing notice) are dropped before
        // the strict chunk parse so they cannot fail the whole turn.
    ...
    async fn create_response_stream_inner(
        ...
        let (response, _sent_bearer) = 'responses_request: loop {
            ...
            span_timing
                .span()
                .record(STATUS_CODE, status.as_u16() as i64);
            span_timing.span().record(SUCCESS, false);
            if status == reqwest::StatusCode::UNAUTHORIZED {
                span_timing.span().record(ERROR, "unauthorized (401)");
                self.record_401_attribution(
        ...
        span_timing
            .span()
            .record(STATUS_CODE, response.status().as_u16() as i64);
        span_timing.span().record(SUCCESS, true);
