...
describe("session/prompt content parts", () => {
  ...
  it("hands a non-image over as a path plus a read-file hint", () => {
    ...
    expect(parts[1]).toEqual({
      type: "text",
      text: expect.stringContaining("/home/demo/report.pdf"),
    });
    expect((parts[1] as { text: string }).text).toContain("read_file");
    expect(parts[2]).toEqual({
      type: "resource_link",
      name: "report.pdf",
      uri: "file:///home/demo/report.pdf",
...
describe("path attachments (native picker and OS drop)", () => {
  ...
  it("keeps a non-image as a path attachment with no bytes", () => {
    const attachment = attachmentFromPath("/home/demo/report.pdf");
    ...
    expect((parts[0] as { text: string }).text).toContain("/home/demo/report.pdf");
    expect(parts[1]).toMatchObject({ type: "resource_link", uri: "file:///home/demo/report.pdf" });
  });

  it("inlines an image only when the bytes were read", () => {
    const inline = attachmentFromPath("/home/demo/shot.png", { data: PNG_BASE64, mediaType: "image/png", size: 4096 });
    ...
    const unread = attachmentFromPath("/home/demo/shot.png");
  ...
  it("names the file from the last path segment", () => {
    ...
    expect(attachmentFromPath("C:\\Users\\demo\\notes.txt").name).toBe("notes.txt");
  });
});
