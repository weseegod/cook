...
describe("sendModelChoice", () => {
  beforeEach(() => {
    ...
    localStorage.removeItem("cook.reasoningEffortByModel");
  ...
  it("sends and remembers the selected reasoning effort", async () => {
    await sendModelChoice("gpt-5", "high");
    ...
    expect(useSessionStore.getState().reasoningEffort).toBe("high");
    expect(JSON.parse(localStorage.getItem("cook.reasoningEffortByModel") ?? "{}")).toEqual({ "gpt-5": "high" });
  });
});
