...
export function turnCompleteNotifyCopy(outcome: TurnNotifyOutcome): { title: string; body: string } {
  if (outcome.kind === "cancelled") {
    return { title: "Let Cook", body: "Turn cancelled" };
  }
  if (outcome.kind === "failed") {
    return { title: "Let Cook", body: "Turn failed" };
  }
  return { title: "Let Cook", body: "Turn completed" };
}
...
export async function notifyTurnComplete(title: string, body: string): Promise<void> {
  const safeTitle = sanitizeNotifyText(title) || "Let Cook";
