#!/usr/bin/env bash
# Install a released Let Cook shell without modifying the Cook CLI.
#
# Usage:
#   bash install.sh 1.0.32
#   curl -fsSL https://github.com/weseegod/thanh/raw/main/frontend/apps/let-cook/scripts/install.sh | bash -s 1.0.32
set -euo pipefail
...
case "$(uname -s)-$(uname -m)" in
  ...
  *) echo "Let Cook v1 supports Linux x86_64 and macOS aarch64." >&2; exit 1 ;;
esac

repo="${THANH_GITHUB_REPO:-weseegod/thanh}"
asset="let-cook-${version}-${platform}.${extension}"
url="https://github.com/${repo}/releases/download/v${version}/${asset}"
download_dir="${COOK_DESKTOP_DOWNLOAD_DIR:-$HOME/.cook/desktop}"
...
if [ "$extension" = "dmg" ]; then
  ...
  echo "Downloaded $final. Open the DMG and drag Let Cook to Applications." >&2
  exit 0
fi
...
final="$download_dir/$asset"
mv -f "$download_tmp" "$final"
bin_dir="${COOK_DESKTOP_BIN_DIR:-$HOME/.local/bin}"
mkdir -p "$bin_dir"
ln -sfn "$final" "$bin_dir/let-cook"
echo "Installed Let Cook to $final" >&2
echo "Launcher: $bin_dir/let-cook" >&2
