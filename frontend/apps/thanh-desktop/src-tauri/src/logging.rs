...
//!
//! Native logs are always appended to `$THANH_HOME/logs/desktop.log` (or the default
//! `~/.thanh/logs/desktop.log`). Verbose ACP tracing is opt-in with
//! `THANH_DESKTOP_TRACE=1`; agent stderr is captured and sanitized into the same log.
...
pub fn init() {
    ...
    let trace = env_truthy("THANH_DESKTOP_TRACE");
    let path = log_path();
    let file = match OpenOptions::new().create(true).append(true).open(&path) {
        Ok(file) => file,
        Err(error) => {
            eprintln!(
                "[thanh-desktop][error] cannot open log {}: {error}",
                path.display()
            );
            return;
        }
...
pub fn log_path() -> PathBuf {
    if let Some(path) = std::env::var_os("THANH_DESKTOP_LOG_FILE") {
        return PathBuf::from(path);
    }
    config_home().join("logs").join("desktop.log")
}
...
fn write_line(level: &str, event: &str, detail: &str, trace_only: bool) {
    ...
    let Some(logger) = LOGGER.get() else {
        eprintln!("[thanh-desktop] {}", line.trim_end());
        return;
    };
